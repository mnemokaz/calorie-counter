(function () {
  ...
  calculate.addEventListener('click', evt => {
    ...
    const maleCalTotal =
      ...
      5 * personalInfo.age +
      5;
    calNorm.innerHTML = Math.floor(
      personalInfo.activity *
        (personalInfo.isMale ? maleCalTotal : femaleCalTotal),
    );
    weightGain.innerHTML = Math.floor(
      personalInfo.activity *
        (personalInfo.isMale ? maleCalTotal : femaleCalTotal) *
        0.15 +
        personalInfo.activity *
    ...
    weightLoss.innerHTML = Math.floor(
      personalInfo.activity *
        (personalInfo.isMale ? maleCalTotal : femaleCalTotal) -
        personalInfo.activity *
          (personalInfo.isMale ? maleCalTotal : femaleCalTotal) *
